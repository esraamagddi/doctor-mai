    ...
    <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div class="group relative rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-white border border-secondary">
        <div class="relative aspect-video bg-gray-900">
          ...
          <img 
            src="{{ $coverImage }}" 
            alt="{{ getLocalized($record->title) }}"
            class="video-cover-{{ $key }} w-full h-full object-cover cursor-pointer transition-opacity duration-300"
            onerror="this.src='https://img.youtube.com/vi/{{ $videoId }}/hqdefault.jpg'"
            onclick="playVideo{{ $key }}()"
          >
          
          <!-- Play Button Overlay -->
          <div class="video-play-btn-{{ $key }} absolute inset-0 flex items-center justify-center bg-black/30 cursor-pointer transition-all duration-300 group-hover:bg-black/40"
            onclick="playVideo{{ $key }}()">
          ...
          <iframe 
            ...
            title="{{ getLocalized($record->title) }}" 
            frameborder="0"
        ...
        <div class="transition-all p-6">
          <h3 class="mb-2 text-lg font-bold text-primary">{{ getLocalized($record->title) }}</h3>
          <p class="text-sm text-gray-600">{{ getLocalized($record->description) }}</p>
        </div>
      </div>

      <script>
        function playVideo{{ $key }}() {
          const cover = document.querySelector('.video-cover-{{ $key }}');
          const playBtn = document.querySelector('.video-play-btn-{{ $key }}');
          const iframe = document.getElementById('video-iframe-{{ $key }}');
