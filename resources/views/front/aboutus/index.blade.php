...
@endphp
@section('title'){{ $seo['meta_title'][app()->getLocale()] ?? Setting()->site_name[app()->getLocale()] ?? '' }}@endsection
@section('description'){{ $seo['meta_description'][app()->getLocale()] ?? Setting()->site_description[app()->getLocale()] ?? '' }}@endsection
@section('og_title'){{ $seo['meta_title'][app()->getLocale()] ?? Setting()->site_name[app()->getLocale()] ?? '' }}@endsection
@section('og_description'){{ $seo['meta_description'][app()->getLocale()] ?? Setting()->site_description[app()->getLocale()] ?? '' }}@endsection
@section('og_url'){{ $seo['canonical'] ?? url()->current() }}@endsection
@section('og_image'){{ !empty($seo['og_image']) ? asset('storage/' . $seo['og_image']) : asset('storage/' . Setting()->logo_light) }}@endsection
@section('twitter_image'){{ !empty($seo['og_image']) ? asset('storage/' . $seo['og_image']) : asset('storage/' . Setting()->logo_light) }}@endsection
@section('twitter_title'){{ $seo['meta_title'][app()->getLocale()] ?? Setting()->site_name[app()->getLocale()] ?? '' }}@endsection
@section('twitter_description'){{ $seo['meta_description'][app()->getLocale()] ?? Setting()->site_description[app()->getLocale()] ?? '' }}@endsection
@section('canonical'){{ $seo['canonical'] ?? url()->current() }}@endsection
@section('meta')
@endsection
    ...
    <div class="container relative z-10 px-6 mx-auto max-w-7xl">
        <div class="mb-16 text-center">
            <h1 class="mb-6 text-4xl font-bold text-gray-900 md:text-5xl lg:text-6xl">
                {{ getSectionHeaders('aboutus')->title[app()->getLocale()] ?? '' }}
            </h1>
            <p class="max-w-3xl mx-auto text-lg text-gray-600 md:text-xl">
                {{ getSectionHeaders('aboutus')->description[app()->getLocale()] ?? '' }}
            </p>
        </div>
    </div>
</section>

@foreach ($founders as $key => $record)
<!-- About Section -->
...
@if($aboutUs && ($aboutUs->education_description[app()->getLocale()] ?? false))
<!-- Qualifications Section -->
<section class="py-24 bg-gradient-to-br from-base to-secondary">
    <div class="container px-6 mx-auto max-w-7xl">
        <div class="mb-16 text-center">
            <h2 class="mb-4 text-3xl font-bold text-gray-900 md:text-4xl">
                {{ __('aboutus.education_qualifications') }}
            </h2>
            <p class="max-w-2xl mx-auto text-lg text-gray-600">
                {!! $aboutUs->education_description[app()->getLocale()] ?? '' !!}
            </p>
        </div>

        <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            @if($aboutUs->education_degree[app()->getLocale()] ?? false)
            <!-- Education Card -->
            <div class="p-8 bg-white shadow-lg rounded-2xl hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-xl font-bold text-gray-900">
                        {{ $aboutUs->education_degree[app()->getLocale()] ?? '' }}
                    </h3>
                </div>
                <p class="text-gray-600">
                    {{ $aboutUs->education_degree_description[app()->getLocale()] ?? '' }}
                </p>
            </div>
            @endif
...
@if($aboutUs && (($aboutUs->experience_years ?? false) || ($aboutUs->philosophy_quote[app()->getLocale()] ?? false)))
<!-- Experience & Philosophy Section -->
<section class="py-24 bg-base">
    <div class="container px-6 mx-auto max-w-7xl">
        <div class="grid gap-16 lg:grid-cols-2">
            
            @if($aboutUs->experience_years ?? false)
            <!-- Experience -->
            <div class="space-y-6">
                <h2 class="text-3xl font-bold text-gray-900 md:text-4xl">
                    <span class="text-primary">{{ $aboutUs->experience_years ?? '' }}</span> {{ __('aboutus.years_of_excellence') }}
                </h2>
                
                @if($aboutUs->treatment_techniques[app()->getLocale()] ?? false)
                <div class="space-y-4">
                    <div class="flex items-start space-x-4 rtl:space-x-reverse">
                        ...
                        <div class="flex-1">
                            <div class="text-gray-700 leading-relaxed prose prose-sm max-w-none">
                                {!! $aboutUs->treatment_techniques[app()->getLocale()] ?? '' !!}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if($aboutUs->philosophy_quote[app()->getLocale()] ?? false)
            <!-- Philosophy -->
            <div class="space-y-6">
                <h2 class="text-3xl font-bold text-gray-900 md:text-4xl">
                    {{ __('aboutus.our_philosophy') }}
                </h2>
                <div class="p-6 border-l-4 border-primary bg-ternary/30 rtl:border-l-0 rtl:border-r-4 rounded-lg">
                    <blockquote class="text-lg italic text-gray-700 leading-relaxed">
                        {!! nl2br(e($aboutUs->philosophy_quote[app()->getLocale()] ?? '')) !!}
                    </blockquote>
                    @if($aboutUs->philosophy_author[app()->getLocale()] ?? false)
                    <cite class="block mt-4 text-sm font-semibold text-primary not-italic">
                        - {{ $aboutUs->philosophy_author[app()->getLocale()] ?? '' }}
                    </cite>
                    @endif
                </div>
            </div>
...
<section class="py-24 bg-gradient-to-br from-secondary to-base">
    <div class="container max-w-4xl px-6 mx-auto text-center">
        <h2 class="mb-6 text-3xl font-bold text-primary md:text-4xl">
            {{ __('aboutus.cta_title') }}
        </h2>
        <p class="mb-8 text-lg text-primary md:text-xl">
            {{ __('aboutus.cta_description') }}
        </p>
