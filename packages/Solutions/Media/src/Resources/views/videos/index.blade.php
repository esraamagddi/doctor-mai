...
@push('title') {{ __('media::messages.videos') }} @endpush
...
<div class="block full">
    <div class="block-title d-flex align-items-center justify-content-between">
        <h2 class="m-0"><strong>{{ __('media::messages.videos') }}</strong></h2>
        <div class="block-options pull-right">
            <a href="{{ route('media.videos.create') }}" class="btn btn-sm btn-primary">
        ...
        <table class="table table-vcenter" id="dataTable">
            <thead class="table-light">
                <tr>
                    ...
                    <th>{{ __('media::messages.title') }}</th>
                    <th>{{ __('media::messages.category') }}</th>
            ...
            <tbody id="sortableBody">
                @foreach($items as $row)
                    <tr draggable="true" data-id="{{ $row->id }}">
                        ...
                        <td>{{ data_get($row->category, 'name.' . app()->getLocale(), data_get($row->category, 'name.en', '-')) }}</td>
                        <td class="text-center">
                            <form action="{{ route('media.videos.toggle', $row) }}" method="post" class="d-inline toggle-form">
                                @csrf
                                @if($row->status)
                        ...
                        <td class="text-end">
                            <div class="btn-group" role="group">
                                <a href="{{ route('media.videos.edit', $row) }}" class="btn btn-sm btn-warning"
                                    data-bs-toggle="tooltip" data-bs-title="{{ __('media::messages.edit') }}"
                                    aria-label="{{ __('media::messages.edit') }}">
                                    <i class="fa fa-pencil"></i>
                                </a>
                            </div>
                            <div class="btn-group" role="group">
                                <form action="{{ route('media.videos.destroy', $row) }}" method="post"
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('saveOrderBtn')?.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(@json(route('media.videos.order', [], false)), {
        method: 'POST',
