...
@push('title') {{ __('media::messages.edit_video') }} @endpush

@section('content')
    <ul class="breadcrumb breadcrumb-top">
        <li><a href="/cp"><i class="fa fa-home"></i></a></li>
        <li><a href="{{ route('media.videos.index') }}">{{ __('media::messages.videos') }}</a></li>
    ...
    <div class="block full">
        <div class="block-title d-flex align-items-center justify-content-between">
            <h2 class="m-0"><strong>{{ __('media::messages.edit') }} :</strong>
                {{ data_get($video->title, app()->getLocale(), data_get($video->title, 'en')) }}</h2>
        ...
        <form action="{{ route('media.videos.update', $video) }}" method="post" class="form-bordered"
                    ...
                    <div class="tab-pane {{ $lang->code === $activeLocale ? 'active' : '' }}" id="tab-{{ $lang->code }}">
                        <div class="form-group">
                            ...
                            <input type="text" name="title[{{ $lang->code }}]" class="form-control"
                                value="{{ old('title.' . $lang->code, $video->title[$lang->code] ?? '') }}"
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description[{{ $lang->code }}]" class="form-control" rows="3"
                                placeholder="{{ __('media::messages.enter_description_' . $lang->code) }}">{{ old('description.' . $lang->code, $video->description[$lang->code] ?? '') }}</textarea>
                        </div>
                    </div>
                @endforeach
            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control">
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}"
                                        {{ old('category_id', $video->category_id ?? null) == $cat->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            @if(!empty($video->image))
                                ...
                                <small class="text-success">Stored file: {{ $video->image }}</small>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea name="embed_code" class="form-control" rows="3"
                            placeholder="{{ __('media::messages.enter_embed_code') }}">{{ old('embed_code', $video->embed_code) }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <label class="switch switch-danger" style="vertical-align: middle;">
                                <input type="checkbox" name="status" value="1" {{ old('status', $video->status) ? 'checked': '' }}>
                                <span></span>
                            </label>
            ...
            <div class="form-group form-actions">
                <button class="btn btn-sm btn-primary"><i class="fa fa-save"></i> {{ __('media::messages.update')
                    }}</button>
