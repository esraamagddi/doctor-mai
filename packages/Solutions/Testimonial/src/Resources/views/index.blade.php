...
@section('content')
    ...
    <div class="block full">
        <div class="block-title d-flex align-items-center justify-content-between">
            <h2 class="m-0"><strong>{{ __('testimonial::messages.testimonial') }}</strong></h2>
            <div class="block-options pull-right">
                <a href="{{ route('testimonial.create') }}" class="btn btn-sm btn-primary">
                    <i class="fa fa-plus me-1"></i> {{ __('testimonial::messages.add') }}
                </a>
            </div>
        </div>
            ...
            <table class="table table-vcenter" id="teamTable">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>{{ __('testimonial::messages.name') }}</th>
                        <th>{{ __('testimonial::messages.job_title') }}</th>
                        <th class="text-center">{{ __('testimonial::messages.status') }}</th>
                        <th class="text-center">{{ __('testimonial::messages.order') }}</th>
                        <th class="text-end">{{ __('testimonial::messages.actions') }}</th>
                    </tr>
                </thead>
                    ...
                    @foreach($items as $row)
                        <tr draggable="true" data-id="{{ $row->id }}">
                            ...
                            <td class="text-center">
                                <form action="{{ route('testimonial.toggle', $row) }}" method="post" class="d-inline toggle-form">
                                    @csrf
                                    @if($row->status)
                                        <span class="badge bg-success">{{ __('testimonial::messages.active') }}</span>
                                    @else
                                        <span class="badge bg-secondary">{{ __('testimonial::messages.inactive') }}</span>
                                    @endif
                                    <button type="submit" class="btn btn-sm btn-outline-secondary ms-2" data-bs-toggle="tooltip"
                                        data-bs-title="{{ __('testimonial::messages.toggle_status') }}"
                                        aria-label="{{ __('testimonial::messages.toggle_status') }}">
                            ...
                            <td class="text-center">
                                <input type="number" class="form-control form-control-sm order-input mx-auto"
                                    ...
                                    aria-label="{{ __('testimonial::messages.item_order', ['id' => $row->id]) }}">
                            </td>

                            <td class="text-end">
                                <div class="btn-group" role="group" aria-label="{{ __('testimonial::messages.actions') }}">
                                    <a href="{{ route('testimonial.edit', $row) }}" class="btn btn-sm btn-warning"
                                        data-bs-toggle="tooltip" data-bs-title="{{ __('testimonial::messages.edit') }}"
                                        aria-label="{{ __('testimonial::messages.edit') }}">
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                </div>
                                <div class="btn-group" role="group">
                                    <form action="{{ route('testimonial.destroy', $row) }}" method="post"
                                        onsubmit="return confirm('{{ __('testimonial::messages.confirm_delete') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" data-bs-toggle="tooltip"
                                            data-bs-title="{{ __('testimonial::messages.delete') }}"
                                            aria-label="{{ __('testimonial::messages.delete') }}">
        ...
        <div class="d-flex flex-column flex-md-row align-items-center justify-content-between small text-muted mt-2">
            <div class="mb-2 mb-md-0">
                ...
                {{ __('testimonial::messages.of') }} <strong>{{ $items->total() }}</strong>
            </div>
            <div>
        ...
        <div class="d-flex gap-2 mt-3">
            <button id="saveOrderBtn" class="btn btn-primary btn-sm" disabled>
                <i class="fa fa-save me-1"></i> {{ __('testimonial::messages.save_order') }}
            </button>
            <small class="text-muted">
                {{ __('testimonial::messages.drag_drop_hint') }}
            </small>
        </div>
    </div>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(@json(route('testimonial.order', [], false)), { // URL نسبي لتفادي http
        method: 'POST',
