...
<ul class="breadcrumb breadcrumb-top">
    ...
    <li>{{ __('services::messages.add') }}</li>
</ul>
...
<div class="block full">
    ...
    <form action="{{ route('services.store') }}" method="post" class="form-bordered" enctype="multipart/form-data">
        @csrf
                ...
                <div class="tab-pane {{ $lang->code === $activeLocale ? 'active' : '' }}" id="tab-{{ $lang->code }}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description[{{ $lang->code }}]" class="form-control" rows="3" id="editor-description-{{ $lang->code }}"
                                  placeholder="{{ __('services::messages.enter_description_'.$lang->code) }}">{{ old('description.'.$lang->code) }}</textarea>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="url" name="link" class="form-control" value="{{ old('link') }}" placeholder="{{ __('services::messages.enter_link') }}">
                </div>
            </div>
        ...
        <div class="form-group form-actions mt-3">
            <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-save"></i> {{ __('services::messages.save') }}</button>
