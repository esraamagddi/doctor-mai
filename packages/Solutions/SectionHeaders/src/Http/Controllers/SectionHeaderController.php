...
namespace Solutions\SectionHeaders\Http\Controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Solutions\SectionHeaders\Models\SectionHeader;
...
class SectionHeaderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $items = SectionHeader::orderBy('order')
            ->orderByDesc('id')
            ->paginate($perPage);

        return view('sectionheaders::index', compact('items'));
    }

    public function children(SectionHeader $header)
    {
        $items = SectionHeader::orderBy('order')
            ->orderByDesc('id')
            ->paginate(12);

        return view('sectionheaders::children', compact('header', 'items'));
    }

    public function create(Request $request)
    {
        [$langs, $activeLocale] = $this->resolveLocales($request);
        $parents = SectionHeader::orderBy('order')->get(); // القايمة الرئيسية
        return view('sectionheaders::create', compact('langs', 'activeLocale', 'parents'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            'slug' => 'required|string|max:255|regex:/^[A-Za-z0-9\.\_\-\/:]+$/|unique:section_headers,slug',
            'eyebrow' => 'nullable|array',
        ...
        $data['order'] = $data['order'] ?? (int) (SectionHeader::max('order') + 1);

        SectionHeader::create($data);
        return redirect()->route('sectionheaders.index')->with('ok', 'Created');
    }

    public function edit(Request $request, SectionHeader $header)
    {
        [$langs, $activeLocale] = $this->resolveLocales($request);
        $parents = SectionHeader::where('id', '!=', $header->id)->orderBy('order')->get();
        ...
        return view('sectionheaders::edit', compact('header', 'langs', 'activeLocale', 'parents'));
    }

    public function update(Request $request, SectionHeader $header)
    {
        [$langs] = $this->resolveLocales($request);
        $default = optional($langs->firstWhere('is_default', 1))->code ?: app()->getLocale() ?: 'en';
        ...
        $rules = [
            'slug' => 'required|string|max:255|regex:/^[A-Za-z0-9\.\_\-\/:]+$/|unique:section_headers,slug,' . $header->id,
            'eyebrow' => 'nullable|array',
            'eyebrow.*' => 'nullable|string',
            'title' => 'required|array',
            'title.' . $default => 'required|string',
        ...
        $header->update($data);
        return redirect()->route('sectionheaders.index')->with('ok', 'Updated');
    }

    public function destroy(SectionHeader $header)
    {
        $header->delete();
        return redirect()->route('sectionheaders.index')->with('ok', 'Deleted');
    }

    public function toggleStatus(SectionHeader $header)
    {
    ...
    public function updateOrder(Request $request)
    {
        ...
        foreach ($rows as $id => $ord) {
            SectionHeader::whereKey($id)->update(['order' => $ord]);
        }
