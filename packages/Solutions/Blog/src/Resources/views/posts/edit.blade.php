...
@push('title') {{ __('blog::messages.edit_post') }} @endpush

@section('content')
<ul class="breadcrumb breadcrumb-top">
    <li><a href="/cp"><i class="fa fa-home"></i></a></li>
    <li><a href="{{ route('blog.posts.index') }}">{{ __('blog::messages.blog') }}</a></li>
    <li>{{ __('blog::messages.edit_post') }}</li>
</ul>

<div class="block full">
    <div class="block-title d-flex align-items-center justify-content-between">
        <h2 class="m-0"><strong>{{ __('blog::messages.edit_post') }}</strong></h2>
    ...
    <form action="{{ route('blog.posts.update', $post) }}" method="post" class="form-bordered"
            ...
            <div class="tab-pane {{ $lang->code === $activeLocale ? 'active' : '' }}" id="tab-{{ $lang->code }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title[{{ $lang->code }}]" class="form-control"
                        value="{{ old('title.' . $lang->code, $post->title[$lang->code] ?? '') }}"
             ...
             <div class="form-group">
                    ...
                    <input type="text" name="author[{{ $lang->code }}]" class="form-control"
                        value="{{ old('author.' . $lang->code, $post->author[$lang->code] ?? '') }}"
                ...
                <div class="form-group">
                    ...
                    <textarea name="description[{{ $lang->code }}]" class="form-control" rows="5"
                        placeholder="{{ __('blog::messages.enter_description', ['lang' => $lang->name]) }}">{{ old('description.' . $lang->code, $post->description[$lang->code] ?? '') }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="editor-content-{{ $lang->code }}" name="content[{{ $lang->code }}]"
                        ...
                        placeholder="{{ __('blog::messages.enter_content', ['lang' => $lang->name]) }}">{{ old('content.' . $lang->code, $post->content[$lang->code] ?? '') }}</textarea>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="category_id" class="form-control">
                        ...
                        <option value="{{ $cat->id }}" {{ old('category_id', $post->category_id ?? '') == $cat->id ?
                            'selected' : '' }}>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="order" value="{{ old('order', $post->order ?? 0) }}"
                        min="0">
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <div>
                        ...
                        <label class="switch switch-danger" style="vertical-align: middle;">
                            <input type="checkbox" name="status" value="1" {{ old('status', $post->status ?? 1) ?
                            'checked' : '' }}>
                            <span></span>
                        </label>
