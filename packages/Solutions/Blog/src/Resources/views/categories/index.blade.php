...
@push('title') {{ __('blog::messages.categories') }} @endpush
...
<ul class="breadcrumb breadcrumb-top">
    ...
    <li>{{ __('blog::messages.categories') }}</li>
</ul>

<div class="block full">
    <div class="block-title d-flex align-items-center justify-content-between">
        <h2 class="m-0"><strong>{{ __('blog::messages.categories') }}</strong></h2>
        <div class="block-options pull-right">
            <a href="{{ route('blog.categories.create') }}" class="btn btn-sm btn-success">
    ...
    <table class="table table-bordered table-striped" id="categoriesTable">
        <thead class="table-light">
            <tr>
                ...
                <th>{{ __('blog::messages.name_en') }}</th>
                <th>{{ __('blog::messages.parent') }}</th>
        ...
        <tbody id="sortableBody">
            ...
            <tr draggable="true" data-id="{{ $item->id }}">
                ...
                <td>{{ data_get($item->name,'en') ?? '-' }}</td>
                <td>{{ optional($item->parent)->name['en'] ?? '-' }}</td>
                <td class="text-center">
                    <form action="{{ route('blog.categories.toggle',$item) }}" method="post" class="d-inline toggle-form">
                        @csrf
                        @if($item->status)
                ...
                <td class="text-center">
                    <input type="number" class="form-control form-control-sm order-input mx-auto"
                        ...
                        aria-label="{{ __('blog::messages.category_order') }}">
                </td>
                <td class="text-end">
                    <div class="btn-group" role="group">
                        <a href="{{ route('blog.categories.edit',$item) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="{{ __('blog::messages.edit') }}">
                            <i class="fa fa-pencil"></i>
                        </a>
                    </div>
                    <div class="btn-group" role="group">
                        <form action="{{ route('blog.categories.destroy',$item) }}" method="post" onsubmit="return confirm('{{ __('blog::messages.delete_confirm') }}')">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="{{ __('blog::messages.delete') }}">
...
<div class="d-flex flex-column flex-md-row align-items-center justify-content-between small text-muted mt-2">
    <div class="mb-2 mb-md-0">
        ...
        {{ __('blog::messages.of') }} <strong>{{ $items->total() }}</strong>
    </div>
    <div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(@json(route('blog.categories.order', [], false)), {
                method: 'POST',
            ...
            saveBtn.innerHTML = '<i class="fa fa-check me-1"></i> {{ __('blog::messages.saved') }}';
            setTimeout(() => saveBtn.innerHTML = old, 1500);
        } catch (e) {
            alert('{{ __('blog::messages.save_failed') }}');
            console.error(e);
            saveBtn.disabled = false;
        }
    });
