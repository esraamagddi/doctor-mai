...
@push('title') {{ __('blog::messages.edit_category') }} @endpush

@section('content')
<ul class="breadcrumb breadcrumb-top">
    <li><a href="/cp"><i class="fa fa-home"></i></a></li>
    <li><a href="{{ route('blog.categories.index') }}">{{ __('blog::messages.blog') }}</a></li>
    <li>{{ __('blog::messages.edit_category') }}</li>
</ul>

<div class="block full">
    <div class="block-title d-flex align-items-center justify-content-between">
        <h2 class="m-0"><strong>{{ __('blog::messages.edit_category') }}</strong></h2>
    ...
    <form action="{{ route('blog.categories.update', $category) }}" method="post" class="form-bordered">
        @csrf @method('PUT')
                ...
                <div class="tab-pane {{ $lang->code === $activeLocale ? 'active' : '' }}" id="tab-{{ $lang->code }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name[{{ $lang->code }}]" class="form-control"
                               value="{{ old('name.'.$lang->code, data_get($category,'name.'.$lang->code)) }}"
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description[{{ $lang->code }}]" class="form-control" rows="4"
                                  placeholder="{{ __('blog::messages.enter_description', ['lang' => $lang->name]) }}">{{ old('description.'.$lang->code, data_get($category,'description.'.$lang->code)) }}</textarea>
                    </div>
                </div>
            @endforeach
        </div>
