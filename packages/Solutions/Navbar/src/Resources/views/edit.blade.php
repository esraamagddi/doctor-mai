...
@push('title') {{ __('navbar::messages.edit_navbar') }} @endpush

@section('content')
    <ul class="breadcrumb breadcrumb-top">
        <li><a href="/cp"><i class="fa fa-home"></i></a></li>
        <li><a href="{{ route('navbar.index') }}">{{ __('navbar::messages.navbar') }}</a></li>
        <li>{{ __('navbar::messages.edit') }}</li>
    </ul>

    <div class="block full">
        <div class="block-title d-flex align-items-center justify-content-between">
            <h2 class="m-0"><strong>{{ __('navbar::messages.edit') }} :</strong> {{ $header->slug }}</h2>
        ...
        <form action="{{ route('navbar.update', $header) }}" method="post" class="form-bordered"
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="slug" class="form-control" value="{{ old('slug', $header->slug) }}"
            ...
            <div class="form-group form-actions">
                <button class="btn btn-sm btn-primary"><i class="fa fa-save"></i>
                    {{ __('navbar::messages.update') }}</button>
                <a href="{{ route('navbar.index') }}" class="btn btn-sm btn-warning"><i class="fa fa-repeat"></i>
                    {{ __('navbar::messages.cancel') }}</a>
            </div>
        </form>
    </div>
