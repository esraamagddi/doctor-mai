...
<div class="block full">
    <div class="block-title d-flex align-items-center justify-content-between">
        ...
        <div class="block-options pull-right">
            <a href="{{ route('pages.create') }}" class="btn btn-sm btn-primary">
                <i class="fa fa-plus me-1"></i> {{ __('pages::messages.add') }}
            </a>
        </div>
    </div>
        ...
        <table class="table table-vcenter" id="dataTable">
            <thead class="table-light">
                <tr>
                    ...
                    <th>{{ __('pages::messages.slug') }}</th>
                    <th>{{ __('pages::messages.title') }}</th>
                    <th class="text-center">{{ __('pages::messages.status') }}</th>
                    <th class="text-center">{{ __('pages::messages.order') }}</th>
                    <th class="text-end">{{ __('pages::messages.actions') }}</th>
                </tr>
            </thead>
            <tbody id="sortableBody">
                @foreach($items as $row)
                    <tr draggable="true" data-id="{{ $row->id }}">
                        ...
                        <td>{{ $row->slug }}</td>
                        <td>
                          {{ data_get($row->title, app()->getLocale(), data_get($row->title, 'en', '-')) }}
                        </td>
                        <td class="text-center">
                            <form action="{{ route('pages.toggle', $row) }}" method="post" class="d-inline toggle-form">
                                @csrf
                                @if($row->status)
                                    <span class="badge bg-success">{{ __('pages::messages.active') }}</span>
                                @else
                                    <span class="badge bg-secondary">{{ __('pages::messages.inactive') }}</span>
                                @endif
                                <button type="submit" class="btn btn-sm btn-outline-secondary ms-2" data-bs-toggle="tooltip"
                                    data-bs-title="{{ __('pages::messages.toggle_status') }}" aria-label="{{ __('pages::messages.toggle_status') }}">
                        ...
                        <td class="text-center">
                            <input type="number" class="form-control form-control-sm order-input mx-auto"
                                ...
                                aria-label="{{ __('pages::messages.item_order', ['id' => $row->id]) }}">
                        </td>
                        <td class="text-end">
                            <div class="btn-group" role="group">
                                <a href="{{ route('pages.edit', $row) }}" class="btn btn-sm btn-warning"
                                    data-bs-toggle="tooltip" data-bs-title="{{ __('pages::messages.edit') }}">
                                    <i class="fa fa-pencil"></i>
                                </a>
                              </div>
                              <div class="btn-group" role="group">
                                <form action="{{ route('pages.destroy', $row) }}" method="post" class="d-inline"
                                    onsubmit="return confirm('{{ __('pages::messages.confirm_delete') }}')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger" data-bs-toggle="tooltip"
                                        data-bs-title="{{ __('pages::messages.delete') }}" aria-label="{{ __('pages::messages.delete') }}">
    ...
    <div class="d-flex flex-column flex-md-row align-items-center justify-content-between small text-muted mt-2">
        <div class="mb-2 mb-md-0">
            ...
            {{ __('pages::messages.of') }} <strong>{{ $items->total() }}</strong>
        </div>
        <div>
    ...
    <div class="d-flex gap-2 mt-3">
        <button id="saveOrderBtn" class="btn btn-primary btn-sm" disabled>
            <i class="fa fa-save me-1"></i> {{ __('pages::messages.save_order') }}
        </button>
        <small class="text-muted">
            {{ __('pages::messages.drag_drop_hint') }}
        </small>
    </div>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const res = await fetch('{{ route('pages.order') }}', {
        method: 'POST',
        headers: {
